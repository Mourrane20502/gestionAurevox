...
const drawCompactDuplicateReceiptBlock = (
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(7.2);
    doc.text("Ref reçu :", left + 6, startY + 16);
    doc.text(refDocLabel, left + 6, startY + 20.5);
    doc.text("Date :", left + 6, startY + 25);
    doc.setFont("helvetica", "normal");
    doc.text(refRecu, left + 23, startY + 16);
    doc.text(refDocValue, left + 23, startY + 20.5);
    doc.text(dateRegStr, left + 23, startY + 25);
    ...
    const condCenterY = doc.internal.pageSize.getHeight() / 2;
    // Place the conditions a bit above center (~15-20 px visually).
    const centerLiftMm = 4.5;
    const condStart = Math.min(Math.max(recapY + 8, condCenterY - 14 - centerLiftMm), condBottom - 32);
